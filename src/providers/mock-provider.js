...
export async function mockModelCall({ prompt, model, chaos = {} }) {
  ...
  // Use fast demo mode (10x faster) or realistic mode
  const demoMode = process.env.DEMO_MODE !== 'false';
  const speedMultiplier = demoMode ? 0.1 : 1.0;
  ...
  // Reduce error probability in demo mode
  const errorMultiplier = demoMode ? 0.3 : 1.0;

  if (chaos.rateLimitSpike && maybe(0.2 * errorMultiplier)) {
    const err = new Error("Rate limit exceeded");
    err.code = 429;
    err.retryable = true;
    err.inputTokens = inputTokens;
    err.outputTokens = 0;
    await sleep(randomInt(10, 40) * speedMultiplier);
    throw err;
  }

  if (chaos.providerOutage && maybe(0.25 * errorMultiplier)) {
    const err = new Error("Provider unavailable");
    err.code = 503;
    err.retryable = true;
    err.inputTokens = inputTokens;
    err.outputTokens = 0;
    await sleep(randomInt(10, 30) * speedMultiplier);
    throw err;
  }

  if (maybe(0.04 * errorMultiplier)) {
  ...
  await sleep(latencyMs * speedMultiplier);
